...
/* old code
if (stepsToMove > 0) {
        //Loop to iterate through steps
        for (int i = 0; i < stepsToMove; i++) { //If I wanted to get fancy I could get rid of the if I'm pretty sure but less clear
            for (int x = 1; x <= 4; x++) { //full step 
                //could probably just do this without the switch and loop but this makes adding a delay easy.
                switch(x) {
                    case 1:
                    s.setA(1)//Set A to on
                    s.setB()//Set Anot to off
                    //Set B to off
                    //Set Bnot to on
                    break;
                    case 2:
        ...
        //Loop to iterate through steps
        for (int i = 0; i > stepsToMove; i--) { //If I wanted to get fancy I could get rid of the if I'm pretty sure but less clear
            for (int x = 1; x <= 4; x++) { //full step backwards
                //could probably just do this without the switch and loop but this makes adding a delay easy.
                switch(x) { //I just reversed the cases to go counterclockwise
                    case 4:
                    //Set A to on
                    //Set Anot to off
                    //Set B to off
